...
default Parquet compression in PySpark is Snappy. Can be changed with spark.conf.set("spark.sql.parquet.compression.codec", "gzip").Other options are gzip (smaller files, slower) and uncompressed.